...
load('PM10Mon_CountyPWC.RData') #PM10Mon_CountyPWC
...
for (i in 1:50){
	...
	test=PMC_DF3[BirthList3]
	DataMatrix[,i]=test$PMC_Week
	rm(BirthList3,test)
}

BirthList4=cbind(data.frame(Combination),data.frame(DataMatrix))
...
BirthList6_PMC=BirthList5[,c(1:3,54:74)]
BirthList6_PMC=BirthList6_PMC[rowSums(!is.na(BirthList6_PMC[,4:24]))>0,]

save(BirthList6_PMC,file='PMC_Col_Exposure_20150318.RData')
